...
def correct_key_names(keys: List[str]) -> List[str]:
    ...
    joined.replace("control + a","ctrl+a")
...
if __name__ == '__main__':
    ...
    KEYBOARD_KEYS = {'\t', '\n', '\r', ' ', '!', '"', '#', '$', '%', '&', "'", '(',
                     ...
                     'command', 'option', 'optionleft', 'optionright','ctrl+a'}
        ...
        if command["transcription"]:
            ...
            if len(parsed) < 2:
                ...
                if base_command == "type this" and len(parsed) > 2:
                    ...
                    # other commands has constant number of arguments
                    
                    execute_command(parsed)
                    print("You said: {}. Executing command...".format(command["transcription"]))
    ...
    print('Time taken for the task ' + str(duration))
